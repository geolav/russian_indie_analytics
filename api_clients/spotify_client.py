...
class SpotifyClient:
    ...
    def search_artist(self, name: str) -> Optional[dict]:
        ...
        """
        data = self._get("/search", {"q": name, "type": "artist", "market": SPOTIFY_MARKET, "limit": 1})
    ...
    def get_artist_top_tracks(self, artist_id: str) -> list[dict]:
        ...
        data = self._get(f"/artists/{artist_id}/top-tracks", {"market": SPOTIFY_MARKET})
    ...
    def get_artist_albums(self, artist_id: str, limit: int = 20) -> list[dict]:
        ...
        data = self._get(
            ...
            {"market": SPOTIFY_MARKET, "include_groups": "album,single", "limit": limit},
        )
        return data.get("items", [])

    def get_album_tracks(self, album_id: str) -> list[dict]:
        """Return tracks inside an album."""
        data = self._get(f"/albums/{album_id}/tracks", {"market": SPOTIFY_MARKET, "limit": 50})
        return data.get("items", [])
    ...
    def get_track(self, track_id: str) -> Optional[dict]:
        ...
        try:
            return self._get(f"/tracks/{track_id}", {"market": SPOTIFY_MARKET})
