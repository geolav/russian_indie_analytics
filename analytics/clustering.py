...
analytics/clustering.py
-----------------------
KMeans clustering of tracks by audio features.
"""
...
def _feature_matrix(df: pd.DataFrame) -> tuple[pd.DataFrame, np.ndarray, StandardScaler]:
    ...
    feature_df = df[cols].dropna()
    if len(feature_df) < 2:
        return feature_df, np.array([]), StandardScaler()
    scaler = StandardScaler()
    X = scaler.fit_transform(feature_df)
    return feature_df, X, scaler
...
def cluster_tracks(df: pd.DataFrame, n_clusters: int = MOOD_CLUSTERS) -> pd.DataFrame:
    ...
    if len(feature_df) < n_clusters:
        logger.warning(f"Not enough samples ({len(feature_df)}) for {n_clusters} clusters")
        df["mood_cluster"] = 0
    ...
    n_clusters = min(n_clusters, len(feature_df) - 1)

    kmeans = KMeans(n_clusters=n_clusters, random_state=RANDOM_STATE, n_init=10)
    labels = kmeans.fit_predict(X)

    # Silhouette score только если есть больше одного кластера и достаточно данных
...
def pca_projection(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(feature_df) < 2 or X.size == 0:
        df["pca_x"] = 0
        df["pca_y"] = 0
        return df
    ...
    proj = pd.DataFrame(components, index=feature_df.index, columns=["pca_x", "pca_y"])
    explained = pca.explained_variance_ratio_
    logger.info("PCA variance explained: PC1=%.1f%%, PC2=%.1f%%", explained[0] * 100, explained[1] * 100)
    return df.join(proj, how="left")
...
def _order_clusters(centers: np.ndarray) -> dict[int, int]:
    energy_idx = AUDIO_FEATURES.index("energy") if "energy" in AUDIO_FEATURES else 1
    order = np.argsort(centers[:, energy_idx])
    return {semantic: int(raw) for semantic, raw in enumerate(order)}