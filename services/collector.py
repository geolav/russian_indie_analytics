...
from api_clients import YandexMusicClient, LastFmClient, GeniusClient
from config.settings import SEED_ARTISTS, MAX_TRACKS_PER_ARTIST, RAW_DIR, REQUEST_DELAY
from data.models import ArtistRecord, TrackRecord
from utils.helpers import ms_to_min, safe_get
from utils.logger import get_logger
...
class DataCollector:
    ...
    def __init__(self) -> None:
        ...
        self.lastfm = LastFmClient()
        self.genius = GeniusClient()
        self._artists: dict[str, ArtistRecord] = {}
        self._tracks: list[TrackRecord] = []  # ← СПИСОК, а не словарь!
    ...
    def collect_all(self, artists: Optional[list[str]] = None) -> None:
        ...
        self._save_to_csv()
        logger.info("Collection complete. %d artists, %d tracks saved.", len(self._artists), len(self._tracks))
    ...
    def _build_track(self, ya_track: dict, artist: ArtistRecord) -> None:
        tid = ya_track.get("id", "")
        track_title = ya_track.get("name", "")

        # Дедупликация
        track_key = f"{artist.name}|{track_title}"
        if track_key in self._seen_tracks:
            logger.debug(f"Duplicate skipped: {track_key}")
            return
        self._seen_tracks.add(track_key)

        # РЕАЛЬНАЯ ПОПУЛЯРНОСТЬ через Last.fm playcount
        real_popularity = 0
        if self.lastfm:
            try:
                real_popularity = self.lastfm.get_track_playcount(artist.name, track_title)
                if real_popularity > 0:
                    logger.info(f"Real playcount for {artist.name} - {track_title}: {real_popularity}")
            except Exception as e:
                logger.debug(f"Failed to get playcount: {e}")

        # РЕАЛЬНЫЙ ГОД через Genius
        release_year = None
        if self.genius:
            try:
                release_year = self.genius.get_song_release_year(artist.name, track_title)
            except Exception as e:
                logger.debug(f"Genius year failed: {e}")

        if not release_year and self.lastfm:
            try:
                release_year = self.lastfm.get_track_release_date(artist.name, track_title)
            except Exception as e:
                logger.debug(f"Last.fm year failed: {e}")
        ...
        track = TrackRecord(
            ...
            release_date="",
            release_year=release_year,
            popularity=real_popularity,  # ← РЕАЛЬНОЕ число прослушиваний!
            duration_ms=ya_track.get("duration_ms", 0),
            duration_min=ms_to_min(ya_track.get("duration_ms", 0)),
            explicit=ya_track.get("explicit", False),
            preview_url="",
    ...
    def _save_to_csv(self) -> None:
        if self._tracks:
            df_tracks = pd.DataFrame([t.__dict__ for t in self._tracks])
            df_tracks = df_tracks.drop_duplicates(subset=["artist", "title"], keep="first")
            df_tracks.to_csv(self.TRACKS_CSV, index=False, encoding="utf-8-sig")
            logger.info("Saved %d tracks → %s", len(df_tracks), self.TRACKS_CSV)
